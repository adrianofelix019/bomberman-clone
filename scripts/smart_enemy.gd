...
func is_cell_safe(cell: Vector2i) -> bool:
	if tile_map.get_cell_source_id(cell) != GROUND_SOURCE_ID:
		return false

	if cell_is_in_bomb_explosion(cell):
		return false

	return true


func cell_is_in_bomb_explosion(cell: Vector2i) -> bool:
	for bomb in get_tree().get_nodes_in_group("bombs"):
		var bomb_cell := tile_map.local_to_map(
			tile_map.to_local(bomb.global_position)
		)
		
		var explosion_cells = tile_map.get_explosion_cells(bomb_cell)

		if cell in explosion_cells:
			return true
	return false
